...
volatile uint32_t g_u32UsbBusResetCount = 0U;
volatile uint32_t g_u32UsbSuspendCount = 0U;
volatile uint32_t g_u32UsbResumeCount = 0U;
volatile uint32_t g_u32UsbSetupCount = 0U;
volatile uint32_t g_u32UsbEp2EventCount = 0U;
volatile uint32_t g_u32UsbSetConfigCount = 0U;
static uint8_t s_au8CtrlInReport[HID_JOYSTICK_REPORT_SIZE] = {0};

static void HID_SetConfigCallback(void)
{
    /*
     * Prime first IN payload right after host sets configuration.
     * Without this kick, some hosts/tools may not trigger EP2 event loop.
     */
    g_u32UsbSetConfigCount++;
    HidTool_SetInReport();
}
...
//------------------------------------------------------------------
    ...
    {
#if (HID_JOYSTICK_USBD_CLEAR_SOF_IN_ISR != 0U)
        /* Clear SOF flag in IRQ (optional). */
...
void HID_Init(void)
{
    ...
    USBD_SetConfigCallback(HID_SetConfigCallback);
}
...
void HID_ClassRequest(void)
{
    ...
    if(buf[0] & 0x80)    /* request data transfer direction */
    {
        ...
        switch(buf[1])
        {
            ...
            case GET_REPORT:
            {
                uint16_t u16ReqLen = (uint16_t)buf[6] | ((uint16_t)buf[7] << 8);
                uint8_t u8TxLen = (uint8_t)((u16ReqLen < HID_JOYSTICK_REPORT_SIZE) ?
                                            u16ReqLen : HID_JOYSTICK_REPORT_SIZE);
                ...
                HidTool_GetLatestInReport(s_au8CtrlInReport, HID_JOYSTICK_REPORT_SIZE);
                USBD_SET_PAYLOAD_LEN(EP1, u8TxLen);
                /* Data stage */
                USBD_PrepareCtrlIn(s_au8CtrlInReport, u8TxLen);
                /* Status stage */
                USBD_PrepareCtrlOut(0, 0);
                break;
            }
